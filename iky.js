...
function clockString(ms) {
    ...
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await ikyy.setStatus(`I'm Userbot 🤖 | Runtime ${runtime(process.uptime())}⏰ | My Owner ${ownerName}`).catch((_) => _);
	...
	case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    ikyy.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    ikyy.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
 ...
 reply(`Penggunaan fitur antidelete :\n\n*${prefix}antidelete [aktif/mati]* (Untuk grup)\n*${prefix}antidelete [ctaktif/ctmati]* (untuk semua kontak)\n*${prefix}antidelete banct 628558xxxxxxx* (banlist kontak)`)
  ...
  break
				...
				case 'setname':
					...
					ikyy.sendMessage(from, '「  SUKSES  」Mengubah Nama Grup my', text, { quoted: fdoc })
					break 
case 'setdesc':
					if (!isGroup) return reply(lang.onlygc())
					if (!isGroupAdmins) return reply(lang.onlygcAdmin())
					...
					ikyy.sendMessage(from, '*「  SUKSES  」Mengubah Desk Grup', text, { quoted: fdoc })
					break   
case 'revoke':
case 'revokegroup':
case 'revokelink': {
...
reply(mess.success)
 ...
 txt += `➸ *ID :* @${i.id.split("@")[0]}\n➸ *Expired*: ${checkExp.days} day(s) ${checkExp.hours} hour(s) ${checkExp.minutes} minute(s)\n\n`
  ...
  Link Qris lain : https://instabio.cc/wannstore
...
ikyy.sendMessage(from,{url:'./'+memek},audio,{mimetype:'audio/mpeg'})
       ...
       case 'tiktokdl':
       case 'tiktoknowm':  
                    if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(lang.erorLink())
 		if (!q) return reply('Linknya?')
 		reply(lang.wait())
		hx.ttdownloader(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `*Link* : ${a.data}`
    nowmm = await getBuffer(nowm)
	ikyy.sendMessage(from,nowmm ,video,{mimetype:'video/mp4',quoted: mek,caption:me})
		})
		})
		break 
		case 'tiktokwm':
		...
		hx.ttdownloader(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `*Link* : ${a.data}`
    wmm = await getBuffer(wm)
		ikyy.sendMessage(from,wmm ,video,{mimetype:'video/mp4',quoted:mek,caption:me})
		})
		})
...
case 'brainly':
					...
					ikyy.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
		            })              
					break
case 'ssweb':
...
case 'nulis':  
                   ...
                   ikyy.sendMessage(from, anu, image, {thumbnail: Buffer.alloc(0),caption: `${lang.success()}\n\nPlease Follow https://instagram.com/achyrr_wann`, quoted: mek})
                   break
//------------------< Level >-------------------
case 'level': 
if (!isGroup) return reply(mess.only.group)
...
case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					try {
						pic = await ikyy.getProfilePicture(mentioneddd[0])
					} catch {
						pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
					}
					seeer = `Nama : *${pushname}`
					thumbb = await getBuffer(pic)
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot by WannBotz 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
                    ikyy.sendMessage(from, thumbb ,image, anuu)
				}
				break
case 'del':
