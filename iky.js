...
fakestatus('anti delete grup aktif')
} else if (q.toLowerCase() === 'disable') {
let anu = antidel.indexOf(from)
antidel.splice(anu, 1)
fs.writeFileSync('./database/antidelete.json', JSON.stringify(antidel))
fakestatus('antiviewonce grup nonaktif')
 } else {
testqq(from, `antiviewonce`)
 }
 break
...
WAKTU        : ${time2}
  ...
  *TERIMA KASIH TELAH ORDER*
          ...
          break
				...
				case 'sewabot':
              gopeynya = `Halo Kak ${pushname} Berikut List Sewa Bot by WANN STORE 
...
BAYAR SESUAI HARGA YA KAK`
   ...
   txtt =`Hai Kak ${pushname} Info Corona Dari Kami Semoga Tersadar \n Bahwa Covid19 Itu Ganas😊 `


 buttons = [{buttonId: "!covidindo", buttonText:{displayText: 'INFO COVID INDONESIA'},type:1},{buttonId: "!covidglobal", buttonText:{displayText:'INFO COVID GLOBAL'},type:1}]
buttonsMessage = {
contentText: `${txtt}`,
footerText: `Selalu Taati 5M`,
buttons: buttons,
headerType: 1
 }
prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{})
...
break
            ...
            anu = await fetchJson(`https://api.lolhuman.xyz/api/corona/indonesia?apikey=PunyaIkyAds`)
            anu1 = anu.result
            ...
            anu = await fetchJson(`https://api.lolhuman.xyz/api/corona/global?apikey=PunyaIkyAds`)
            anu1 = anu.result
...
if (budy.includes(`assalamualaikum`)) {
		...
		if (budy.includes(`Assalamualaikum`)) {
                  ...
                  fakestatus(`Ya Ada Apa Memanggil Owner Ku ? Owner Ku Sibuk Mendua:v`)
                  }
